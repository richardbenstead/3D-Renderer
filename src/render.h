...
class Render {
  public:
    static void drawImage(auto &canvas, auto const &scene) {
        ...
        canvas.fillScreen(colRGB(0.0, 0.0, 0.0));
        for (const Triangle &tri : triangles) {
            // the virtual screen -1->1 maps to the image
            auto toScreen = [&](Vec2f const &point, Vec2f const &texCoord) -> RenderVertex {
                return RenderVertex{Vec2i{canvas.width() * (1 + point[0]) / 2.0,
                                          canvas.height() * (1 + point[1]) / 2.0},
                                   texCoord};
            };
            ...
            if (tri.shader->isTex()) {
                TextureShader* ts = static_cast<TextureShader*>(tri.shader.get());
                canvas.template drawFilledTriangle<TextureShader>(rv1, rv2, rv3, tri.vertexDist, *ts);
            } else {
                BasicShader* bs = static_cast<BasicShader*>(tri.shader.get());
                canvas.template drawFilledTriangle<BasicShader>(rv1, rv2, rv3, tri.vertexDist, *bs);
            }
        }
    }
};
